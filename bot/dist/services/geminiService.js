...
exports.VoiceParseError = void 0;
exports.parseVoiceNote = parseVoiceNote;
const generative_ai_1 = require("@google/generative-ai");
const env_1 = require("../config/env");
const timezone_1 = require("../utils/timezone");
const dateParser_1 = require("../utils/dateParser");
class VoiceParseError extends Error {
    constructor(message) {
        super(message);
        this.name = "VoiceParseError";
    }
}
exports.VoiceParseError = VoiceParseError;
function buildSystemPrompt(nowDate, nowTime, timezone) {
...
async function tryModel(apiKey, modelName, base64Audio, mimeType, prompt) {
    ...
    if (!text)
        throw new VoiceParseError("Gemini returned an empty response.");
    return text;
}
function nullishString(value) {
...
function normalizeParsed(raw, fallbackTz) {
    const reminderTypeRaw = nullishString(raw.reminderType) ?? nullishString(raw.type);
    let reminderType = null;
    if (reminderTypeRaw === "one_time" || reminderTypeRaw === "onetime")
        reminderType = "one_time";
    if (reminderTypeRaw === "recurring" || reminderTypeRaw === "cycle")
...
async function parseVoiceNote(audioBuffer, mimeType = "audio/ogg", context = {
    date: "",
    time: "",
    ...
    const timezone = context.timezone || timezone_1.DEFAULT_TIMEZONE;
    ...
    for (const modelName of MODEL_FALLBACK_CHAIN) {
        for (let attempt = 1; attempt <= 2; attempt++) {
            try {
                ...
                const rawText = await tryModel(env_1.env.GEMINI_API_KEY, modelName, base64Audio, mimeType, prompt);
                let parsed;
                try {
                    ...
                    throw new VoiceParseError(`Failed to parse Gemini JSON response: ${rawText.slice(0, 200)}`);
                }
                if (typeof parsed !== "object" || parsed === null) {
                    throw new VoiceParseError(`Gemini response was not an object: ${rawText.slice(0, 200)}`);
                }
                const normalized = normalizeParsed(parsed, timezone);
    ...
    throw new VoiceParseError("All Gemini models are currently unavailable. Please try again in a moment.");
}
